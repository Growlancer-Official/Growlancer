...
| AI | Gemini (OpenAI-compatible gateway) via raw REST fetch in edge functions (AI SDK deps unused) |
...
`VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, optional `VITE_SENTRY_DSN`, `VITE_APP_VERSION`. Server-side (edge functions): `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `GEMINI_API_KEY`, `GEMINI_BASE_URL`, `GEMINI_MODEL`, `RAZORPAY_KEY_ID/SECRET/ACCOUNT_NUMBER`, `RAZORPAY_WEBHOOK_SECRET`, `PAYPAL_*`, `ADMIN_SIGNUP_SECRET`, `CRON_SECRET`, `APP_URL`.
...
- **AI runs on Gemini** (OpenAI-compatible gateway; env-driven `GEMINI_BASE_URL`/`API_KEY`/`MODEL`). All AI functions fail gracefully to deterministic behavior when the gateway is unreachable.
  - **AI Chat Assistant** (`ai-assistant/index.ts`) — Gemini, server-side, role-specific system prompts (freelancer vs client). **Unlimited for both roles** (clients are 100% free; no usage-gating since 2026-08-07). Rate limited 30/60s. Streams SSE to `AIChatSupport.tsx`.
  ...
  - **Talent matching** (`ai-matching/index.ts` + client `src/lib/aiMatching.ts`) — real-time deterministic scoring (category overlap, budget, rating) + optional Gemini semantic boost with graceful fallback. Writes to `ai_matches`.
...
- **Disposable-email blocklist** (comprehensive): `is_disposable_email_domain(p_domain)` holds the full **8,417-domain** server-side list (migration `20261215000002_comprehensive_disposable_blocklist.sql`; subdomain-aware — `foo.mailinator.com` matches `mailinator.com`). Enforced in **3 layers**: (1) client inline check via `src/lib/disposableEmails.ts` curated list, (2) `SignupModal` server-authoritative RPC pre-check before submit, (3) `block_disposable_email_signup` trigger on `auth.users` INSERT (cannot be bypassed). Refresh the list by downloading a fresh community dataset and running `node scripts/gen-disposable-blocklist.mjs <path>` (regenerates the migration).
