...
- **⚠️ CONFIRMED BUG — UI withdrawals never actually pay out.** `withdrawalService.createWithdrawal` (`src/lib/withdrawal.ts:106-160`, called by `WalletPage.tsx:417`) calls `hold_wallet_funds` then **directly inserts** a `withdrawals` row (`fee:0`, `net_amount:amount`, `status:'pending'`) — it never calls the `withdrawal` edge function POST that executes the real RazorpayX `/payouts` / PayPal payout call. Only GET history uses the edge function. So withdrawals sit `pending`, and `process_stale_withdrawals` cron auto-fails them after 72h and returns the funds. `process_withdrawal_complete` (which would clear `pending_balance` on completion) is never called either, so even completed payouts leave the held amount stuck in `pending_balance`.
- **⚠️ Milestone funding bug:** the `razorpay` fn's `create_order` reads `escrow.milestones`, but there is **no `milestones` column on `escrow`** → the select fails → sum=0 → falls back to charging the full contract amount. Milestones actually live in `contracts.milestones` (JSONB). Partial-milestone funding silently overcharges.
...
15. **`notificationPreferencesService.resetToDefaults`** upserts `{ categories: ... }` but the table column is `preferences` — the reset always fails.
16. **`getUserReviews` averages only the latest 50 reviews** (disagrees with DB-side `get_reputation_stats`).
17. **`invitesService.resend` rewrites `created_at`** (history mutation).
18. **Internship GET/PATCH endpoints are unauthenticated** in `internship-applications` — anyone can read all applicant PII and change statuses/meet links. Don't extend; fix with auth when touching.
