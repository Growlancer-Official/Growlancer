    ...
    <div className="max-w-[100rem] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
        <div>
          ...
          <p className="text-slate-600 text-[10px] sm:text-xs mt-0.5">Manage your projects and freelancers all in one place.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-3">
        {statCards.map((stat, index) => (
          ...
          >
            ...
            <p className="text-slate-600 text-[10px] sm:text-[11px]">{stat.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <div className="bg-white rounded-xl p-3 border border-slate-100">
          <div className="flex items-center justify-between mb-2">
            ...
            <Link to="/client/projects" className="text-emerald-600 font-medium text-[10px] hover:underline">
              View All →
            </Link>
          </div>
          ...
          {recentProjects.length > 0 ? (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-slate-900 text-[11px] sm:text-xs truncate">{project.title}</h4>
                      <p className="text-[10px] text-slate-500 mt-0.5 line-clamp-1">{project.description}</p>
                    </div>
            ...
            <div className="text-center py-4">
              ...
              <p className="text-slate-600 mb-2 max-w-xs mx-auto text-[10px]">No projects yet. Create your first one.</p>
              <Link
                to="/client/post"
        ...
        <div className="bg-white rounded-xl p-3 border border-slate-100">
          <div className="flex items-center justify-between mb-2">
            ...
            <Link to="/client/contracts" className="text-emerald-600 font-medium text-[10px] hover:underline">
              View All →
            </Link>
          </div>
          ...
          {recentContracts.length > 0 ? (
            <div className="space-y-1.5">
              {recentContracts.slice(0, 4).map((contract) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-slate-900 text-[11px] sm:text-xs truncate">
            ...
            <div className="text-center py-4">
              ...
              <p className="text-slate-600 mb-2 max-w-xs mx-auto text-[10px]">No active contracts yet.</p>
              <div className="flex gap-1.5 justify-center">
                <Link
                  to="/client/proposals"
                  className="inline-flex items-center justify-center gap-1.5 px-2.5 py-1 bg-emerald-600 text-white text-[10px] font-medium rounded-lg hover:bg-emerald-700 transition-colors"
                >
                ...
                <Link
                  to="/client/matches"
                  className="inline-flex items-center gap-1.5 px-2.5 py-1 border border-slate-200 text-slate-700 rounded-lg font-medium hover:bg-slate-50 transition-colors text-[10px]"
                >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-3">
        ...
        <div className="bg-white rounded-xl p-3 border border-slate-100">
          ...
          <div className="flex flex-col gap-0.5">
            <Link to="/client/ai-assistant" className="text-emerald-600 font-medium text-[10px] hover:underline">
              Chat with AI Assistant →
            </Link>
            <Link to="/help-center" className="text-emerald-600 font-medium text-[10px] hover:underline">
              Visit Help Center →
            </Link>
          </div>
        </div>
