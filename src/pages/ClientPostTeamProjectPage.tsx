  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <TipNote tone="info" className="mb-6">
        <strong>How it works:</strong> har role ke liye ek <strong>independent contract</strong> banega — apna escrow, apne milestones, apna dispute. Ek member ka issue baaki team ko affect nahi karta. Commission (5%) har contract par lagta hai, koi alag "team fee" nahi.
      </TipNote>
      ...
      <form onSubmit={handleCreateProject} className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={addRole}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-emerald-600 text-emerald-600 text-sm font-semibold hover:bg-emerald-50 transition-colors"
            >
            ...
            <div key={role.key} className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm space-y-4">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-violet-600 text-white text-sm font-semibold hover:bg-violet-700 disabled:opacity-60 transition-colors"
                >
              ...
              {role.suggestions.length > 0 && (
                <div className="border-t border-slate-100 pt-4">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Suggested Freelancers for this role</p>
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {role.suggestions.slice(0, 6).map((s) => (
                      <div key={s.freelancer_id} className="flex items-center justify-between gap-3 p-3 rounded-xl border border-slate-100 bg-slate-50/50">
                        ...
                        {s.hourly_rate > 0 && (
                          <span className="text-xs font-semibold text-slate-600 shrink-0">{formatCurrency(s.hourly_rate)}/hr</span>
                        )}
                      </div>
                    ))}
                  </div>
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <button
            type="submit"
            disabled={saving}
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-700 disabled:opacity-60 shadow-lg"
          >
