  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-slate-900">Waitlist</h1>
        ...
        <button
          ...
          disabled={loading}
          className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-medium hover:bg-slate-50 transition-colors disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
          ...
          <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">Total Signups</p>
        </div>
        <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
          ...
          <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">Countries Reached</p>
        </div>
        <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
          <div className="w-10 h-10 rounded-xl bg-purple-100 text-purple-600 flex items-center justify-center mb-3">
            <MapPin className="w-5 h-5" />
          </div>
          <p className="text-2xl font-bold text-slate-900 truncate">{topCountry}</p>
          <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">Top Country</p>
        </div>
        <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
          <div className="w-10 h-10 rounded-xl bg-amber-100 text-amber-600 flex items-center justify-center mb-3">
            <Mail className="w-5 h-5" />
          </div>
          <p className="text-2xl font-bold text-slate-900">{countryFilter === 'all' ? totalEntries : countryCounts[countryFilter] || 0}</p>
          <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">Current View</p>
        </div>
      </div>
                ...
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-medium text-slate-700 flex items-center gap-2">
                    ...
                    {countryFilter === country && (
                      <span className="text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full font-bold">Filtered</span>
                    )}
                  </span>
        ...
        <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
            />
          </div>
          <select
            value={countryFilter}
            onChange={(e) => setCountryFilter(e.target.value)}
            className="px-3 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
          >
              ...
              {loading ? (
                ...
                filtered.map((e) => (
                  <tr key={e.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="px-6 py-3 text-slate-500 capitalize">{e.signup_source || '—'}</td>
